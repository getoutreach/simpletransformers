...
from simpletransformers.classification.transformer_models.bert_model_additional_features import (
    BertForSequenceClassificationAddFeatures,
)
...
class ClassificationModelAddFeatures:
    def __init__(
        self,
        model_type,
        model_name,
        ...
        **kwargs,
        ...
        MODEL_CLASSES = {
            "bert": (BertConfig, BertForSequenceClassificationAddFeatures, BertTokenizer),
        }
        ...
        if self.weight:
            self.model = model_class.from_pretrained(
                model_name,
                config=self.config,
                additional_features_size=args['additional_features_size'],
                weight=torch.Tensor(self.weight).to(self.device),
                **kwargs,
            )
        else:
            self.model = model_class.from_pretrained(
                model_name, config=self.config, additional_features_size=args['additional_features_size'], **kwargs
            )
    ...
    def train_model(
        ...
        **kwargs,
        ...
        assert "text_a" in train_df.columns and "text_b" in train_df.columns and 'addfeatures' in train_df.columns
        train_examples = [
            InputExample(i, text_a, text_b, label, addfeatures=addfeatures)
            for i, (text_a, text_b, label, addfeatures) in enumerate(
                zip(train_df["text_a"], train_df["text_b"], train_df["labels"], train_df["addfeatures"])
            )
        ]
    ...
    def evaluate(self, eval_df, output_dir, multi_label=False, prefix="", **kwargs):
        ...
        assert "text_a" in eval_df.columns and "text_b" in eval_df.columns and "addfeatures" in eval_df.columns
        eval_examples = [
            InputExample(i, text_a, text_b, label, addfeatures)
            for i, (text_a, text_b, label, addfeatures) in enumerate(
                zip(eval_df["text_a"], eval_df["text_b"], eval_df["labels"], eval_df["addfeatures"])
            )
        ]
    ...
    def load_and_cache_examples(
        ...
        all_addfeatures = torch.tensor([f.addfeatures for f in features], dtype=torch.float)
        ...
        dataset = TensorDataset(
            all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_addfeatures
        )
    ...
    def _get_inputs_dict(self, batch):
        inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3], "addfeatures": batch[4]}
